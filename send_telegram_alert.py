...
3. 단원별 정량적 데이터(점수, 시간, 오답)를 바탕으로 학생의 '강점'과 '약점(보완할 점)'을 과외 선생님의 시선에서 전문적으로 분석해주세요.
4. 밀린 학습을 한꺼번에 해내려는 태도나 집중력 등, 데이터 이면의 모습까지 정성적으로 능동적인 판단을 더해 평가해주세요.
5. 어제 학습 여부에 따라, 칭찬 스티커 부여나 따뜻한 독려를 학부모님께 제안하며 마무리해주세요.
6. 이모지를 적절히 사용하여 읽기 좋게 3~4문단 정도로 작성해주세요."""
